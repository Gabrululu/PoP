...
function seededInt(seed: number, max: number) {
  // deterministic pseudo-random from a seed so values stay stable per pixel
...
export default function PixelInspector({ pixel, balance, onClose, onReclaim }: Props) {
  ...
  const { owner, minutes, history } = useMemo(() => {
    ...
    const hist = [
      {
        color: PALETTE[seededInt(seed + 2, PALETTE.length)],
        address: shortAddr(DEMO_ADDRESSES[seededInt(seed + 3, DEMO_ADDRESSES.length)]),
        ago: `${seededInt(seed + 4, 15) + 2} min`,
      },
      {
        color: PALETTE[seededInt(seed + 5, PALETTE.length)],
        address: shortAddr(DEMO_ADDRESSES[seededInt(seed + 6, DEMO_ADDRESSES.length)]),
        ago: `${seededInt(seed + 7, 30) + 20} min`,
      },
      {
        color: PALETTE[seededInt(seed + 8, PALETTE.length)],
        address: shortAddr(DEMO_ADDRESSES[seededInt(seed + 9, DEMO_ADDRESSES.length)]),
        ago: `${seededInt(seed + 10, 30) + 55} min`,
      },
    ];
    ...
    <>
      ...
      >
        ...
        <div style={{ background: '#0c0c14', border: '0.5px solid #2a2a4a', borderRadius: 10, padding: '10px 12px', marginBottom: 10 }}>
          <div style={{ fontSize: 10, color: '#5a5a8a', marginBottom: 4 }}>propietario actual</div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <span style={{ fontSize: 13, color: '#35d07f', fontWeight: 500 }}>{owner}</span>
            <span style={{ fontSize: 10, color: '#5a5a8a' }}>hace {minutes} min</span>
          </div>
        </div>

        {/* History */}
        <div style={{ marginBottom: 14 }}>
          <div style={{ fontSize: 10, color: '#5a5a8a', marginBottom: 6 }}>últimos pintores</div>
        ...
        >
          {canAfford ? 'Reclamar este píxel — 0.01 cUSD' : 'Balance insuficiente'}
        </button>
      </div>
    </>
  );
